...
@Repository	// 메모리 할당 = Spring이 관리가 가능하도록
/*
	Spring => 클래스 관리 (Container)
				| 의존성이 낮은 프로그램 (수정 시 다른 클래스에 영향이 없는 프로그램)
		*** 어떤 클래스 관리
			VO / Entity
			-----------
				|
		...
		=> 동작
			요청 === DispatcherServlet ==== HandlerMapping
											...
											ViewResolver
											   ...
											   --- 받는 데이터로 출력
		=> 자동 세팅 : prefix / suffix
		=> Framework에 있는 XML (세팅 파일을 자동처리)
			--------- 간결하게 만든 프로그램
		=> 어노테이션이 존재 => @RequestMapping 사라진다
*/
// #{name} 'hongildong'          ${name} gongildong
// 일반 데이터는 #{}, 컬럼/테이블 => ${}
/*
		USER (브라우저)
			|
